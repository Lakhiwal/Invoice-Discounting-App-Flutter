...
class Analytics extends _$Analytics {
  ...
  Future<AnalyticsState> _loadInitialData() async {
    try {
      final data = await PortfolioCache.getPortfolio(forceRefresh: false);
  ...
  void setTimeFilter(String filter) async {
...
AnalyticsState _calculateMetricsIsolate(AnalyticsState state) {
  ...
  // 2. Base metrics
  double totalInvested = filteredActive.fold(0.0, (s, i) => s + _dbl(i['amount']));
  
  double totalReturns = filteredRepaid.fold(0.0, (s, i) {
  ...
  int avgTenure = 0;
  if (all.isNotEmpty) {
    final totalTenure = all.fold(0, (s, i) => s + (i['tenure_days'] as int? ?? 0));
  ...
  final monthlyEarnings = _computeMonthlyEarnings(filteredActive, filteredRepaid);
  ...
  // 4. Health Score
  final health = _calculateHealth(filteredActive, totalInvested, avgYield, sectorData, riskBuckets);
...
// Helpers
List<Map<String, dynamic>> _applyTimeFilter(List<Map<String, dynamic>> items, String filter) {
...
List<Map<String, dynamic>> _computeSectorData(List<Map<String, dynamic>> active) {
...
List<Map<String, dynamic>> _computeMaturityData(List<Map<String, dynamic>> active) {
...
Map<String, double> _computeRiskBuckets(List<Map<String, dynamic>> active) {
  ...
  for (final inv in active) {
    ...
    if (daysLeft < 0) overdue += amount;
    else if (daysLeft < 7) urgent += amount;
    else if (daysLeft < 30) approaching += amount;
    else safe += amount;
  }
  return {'safe': safe, 'approaching': approaching, 'urgent': urgent, 'overdue': overdue};
}

List<Map<String, dynamic>> _computeMonthlyEarnings(List<Map<String, dynamic>> active, List<Map<String, dynamic>> repaid) {
  ...
  for (final inv in repaid) {
    ...
    final profit = _dbl(inv['actual_returns']) > 0 ? _dbl(inv['actual_returns']) : _dbl(inv['expected_profit']);
  ...
  return allKeys.map((key) {
    ...
    return {
      ...
      'isPast': dt.isBefore(DateTime(DateTime.now().year, DateTime.now().month, 1)),
    };
  }).toList();
}
...
_HealthResult _calculateHealth(List<Map<String, dynamic>> active, double totalInvested, double avgYield, List<Map<String, dynamic>> sectorData, Map<String, double> risk) {
  if (active.isEmpty) {
    return _HealthResult(0, 'No data', Colors.grey, []);
  }
  final sectorCount = sectorData.length;
  final maxConcentration = totalInvested > 0
      ? sectorData.fold(0.0, (m, s) => math.max(m, (s['amount'] as double) / totalInvested))
      : 1.0;
  double divScore = (sectorCount >= 4 && maxConcentration < 0.4) ? 100 : (sectorCount >= 3 ? 75 : (sectorCount >= 2 ? 50 : 20));
  final yieldScore = (avgYield / 12.0 * 100).clamp(0.0, 100.0);
  final totalAmt = totalInvested > 0 ? totalInvested : 1;
  final safeWeighted = (risk['safe'] ?? 0) * 1.0 + (risk['approaching'] ?? 0) * 0.7 + (risk['urgent'] ?? 0) * 0.3;
  final safetyScore = (safeWeighted / totalAmt * 100).clamp(0.0, 100.0);
  final score = (divScore * 0.4 + yieldScore * 0.3 + safetyScore * 0.3).round();

  String label;
  Color color;
  if (score >= 80) { label = 'Excellent'; color = const Color(0xFF12B76A); }
  else if (score >= 60) { label = 'Good'; color = const Color(0xFFF59E0B); }
  else if (score >= 40) { label = 'Fair'; color = const Color(0xFFF97316); }
  else { label = 'Needs attention'; color = const Color(0xFFEF4444); }

  final factors = [
    if (divScore >= 75) 'Diversified' else 'Low diversity',
    if (yieldScore >= 80) 'Strong yield' else 'Yield below target',
    if (safetyScore >= 80) 'No overdue' else if ((risk['overdue'] ?? 0) > 0) 'Has overdue' else 'Some urgent',
  ];

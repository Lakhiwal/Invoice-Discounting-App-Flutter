...
class Marketplace extends _$Marketplace {
  ...
  static const List<String> statusFilters = ['All', 'Available', 'Partially Funded'];
  static const List<String> quickFilters = ['High ROI', 'Short Tenure', 'Almost Funded'];

  @override
  FutureOr<MarketplaceState> build() async {
    // Initial state setup
    final initialState = MarketplaceState(
      allInvoices: [],
      filtered: [],
    );
  ...
  Future<MarketplaceState> _fetchInvoices(MarketplaceState current, {bool forceRefresh = false}) async {
    int targetPage = forceRefresh ? 1 : current.page;
    ...
    try {
      ...
      final incoming = data.map<InvoiceItem>((e) => InvoiceItem.fromMap(e as Map<String, dynamic>)).toList();
      
      List<InvoiceItem> updatedAll = forceRefresh ? [] : List.from(current.allInvoices);
      
      for (var item in incoming) {
        final idx = updatedAll.indexWhere((i) => i.id == item.id);
        if (idx != -1) updatedAll[idx] = item;
        else updatedAll.add(item);
      }
  ...
  Future<void> loadMore() async {
    ...
    state = await AsyncValue.guard(() => _fetchInvoices(state.value!, forceRefresh: false));
  }
  ...
  Future<void> refresh({bool silent = false}) async {
    ...
    final newState = await AsyncValue.guard(() => _fetchInvoices(const MarketplaceState(), forceRefresh: true));
  ...
  void toggleQuickFilter(String f) {
    ...
    if (current.activeQuickFilter == f) {
      next = current.copyWith(
        ...
        minRoi: 0, maxRoi: 30, minDays: 0, maxDays: 365, minFunding: 0, maxFunding: 100,
      );
    } else {
      next = current.copyWith(
        activeQuickFilter: f,
  ...
  void _triggerFiltering() async {
    final current = state.value;
    if (current == null) return;
    state = await AsyncValue.guard(() => _applyFiltersToState(current));
  }

  Future<MarketplaceState> _applyFiltersToState(MarketplaceState current) async {
    final generation = current.filterGeneration + 1;
    final params = {
      'invoices': current.allInvoices.map((i) => {
        'id': i.id, 'company': i.company, 'status': i.status, 'statusDisplay': i.statusDisplay,
        'roi': i.roi, 'tenureDays': i.tenureDays, 'remainingAmount': i.remainingAmount, 'fundingPct': i.fundingPct,
      }).toList(),
      'status': current.selectedStatus, 'minRoi': current.minRoi, 'maxRoi': current.maxRoi, 
      'minDays': current.minDays, 'maxDays': current.maxDays,
      'minFunding': current.minFunding, 'maxFunding': current.maxFunding, 
      'sortBy': current.sortBy, 'query': current.searchQuery,
    };
    ...
    final filtered = rawResult.map((m) => idMap[m['id']]).whereType<InvoiceItem>().toList();
...
List<Map<String, dynamic>> _filterInvoicesIsolate(Map<String, dynamic> params) {
  ...
  if (selectedStatus == 'Available') {
    result = result.where((i) => (i['status'] ?? '').toString().toLowerCase() == 'available').toList();
  ...
  result = result.where((i) {
    ...
    return roi >= params['minRoi'] && roi <= params['maxRoi'] &&
           tenure >= params['minDays'] && tenure <= params['maxDays'] &&
           funding >= params['minFunding'] && funding <= params['maxFunding'];
  }).toList();

  if (query.isNotEmpty) {
    result = result.where((i) => (i['company'] as String).toLowerCase().contains(query)).toList();
  }

  final sortBy = params['sortBy'] as String;
  if (sortBy == 'roi_high') result.sort((a,b) => (b['roi'] as num).compareTo(a['roi'] as num));
  else if (sortBy == 'days_low') result.sort((a,b) => (a['tenureDays'] as num).compareTo(b['tenureDays'] as num));
  else if (sortBy == 'amount_high') result.sort((a,b) => (b['remainingAmount'] as num).compareTo(a['remainingAmount'] as num));

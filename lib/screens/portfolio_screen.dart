...
// Black mode card helpers (same pattern as analytics)
  ...
  Widget build(BuildContext context) {
    ...
    if (investments.isEmpty) {
      return Center(
        ...
        Text(isRepaid ? 'No repaid investments' : 'No active investments',
            style: TextStyle(
...
class _InvestmentCard extends StatelessWidget {
  ...
  const _InvestmentCard(
      {required this.inv, required this.isRepaid, this.isBlackMode = false});
        ...
        if (context.mounted) {
          showModalBottomSheet(
            ...
            builder: (_) => _InvestmentDetailSheet(
                inv: inv, isRepaid: isRepaid, isBlackMode: isBlackMode),
          );
        }
      },
      child: Container(
        ...
        decoration: BoxDecoration(
            ...
            border: Border.all(color: _cardBorder(context, isBlackMode, 0.1))),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          // ── Invested amount (prominent) ────────────────────────
          Text('₹${fmtAmount(invested)}',
              style: TextStyle(
                  color: colorScheme.onSurface,
                  fontWeight: FontWeight.w800,
                  fontSize: 17)),
          ...
          // ── Metrics row: rate, date ────────────────────────────
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            _Metric(
                label: 'Investor Rate', value: '${inv['investor_rate']}% p.a.'),
            _Metric(
                label: isRepaid ? 'Settled On' : 'Due Date',
                value: inv['payment_date']?.toString() ?? '--'),
          ]),

          // ── Funding progress (if approved_amount available) ──
          if (approved > 0) ...[
            ...
            ClipRRect(
                ...
                child: LinearProgressIndicator(
                    ...
                    valueColor: AlwaysStoppedAnimation(isFull
                        ? AppColors.emerald(context)
                        : colorScheme.primary))),
            const SizedBox(height: 5),
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text('Funded ${fundedPct.toStringAsFixed(0)}%',
                  style: TextStyle(
                      color: isFull
                          ? AppColors.emerald(context)
                          : colorScheme.primary,
          ...
          // ── Tenure progress bar (active only) ─────────────────
          if (!isRepaid) ...[
            const SizedBox(height: 16),
            ClipRRect(
          ...
          Row(mainAxisAlignment: MainAxisAlignment.end, children: [
            ...
            Icon(Icons.arrow_forward_ios_rounded,
                size: 9, color: colorScheme.primary.withValues(alpha: 0.7)),
          ]),
        ]),
      ),
    );
    ...
    return DraggableScrollableSheet(
      ...
      builder: (ctx, scrollCtrl) => ListView(
        ...
        children: [
          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Expanded(
                child: Column(
                    ...
                    children: [
                  Text(inv['company']?.toString() ?? 'Investment',
                      style: TextStyle(
                          color: colorScheme.onSurface,
                          fontSize: 22,
                          fontWeight: FontWeight.w800,
          ...
          _DetailCard(isBlackMode: isBlackMode, rows: [
            ...
            _Row('Reference / Funding ID', inv['id']?.toString() ?? '--',
                onCopy: () => _copy(context, inv['id']?.toString() ?? '',
                    'Reference ID copied')),
          ...
          _DetailCard(isBlackMode: isBlackMode, rows: [
            _Row('Investor Rate (p.a.)',
                ...
                valueColor: AppColors.emerald(context), bold: true),
            if (!isRepaid) ...[
              ...
              _Row('Expected Payout', '₹${_fmtFull(_expectedPayout)}',
                  bold: true),
              _Row('Payment Due Date', inv['payment_date']?.toString() ?? '--'),
              _Row(
                  'Days Remaining',
                  _isOverdue
